...
public class ResidentDAO {
    ...
    public int countResidentsByBuilding(Long buildingId) {
        String sql = "SELECT COUNT(DISTINCT r.id) FROM residents r JOIN contracts c ON c.resident_id = r.id JOIN apartments a ON c.apartment_id = a.id JOIN floors f ON a.floor_id = f.id WHERE f.building_id = ? AND r.is_deleted = 0 AND c.status = 'ACTIVE'";
        try (Connection conn = Db_connection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            if (buildingId == null) {
